...
bool check(ll val)
{
    ...
    t.upd(1, cc, 1, e[1].y1, e[1].y2-1, 1);
    FOR(i, 2, cur)
    {
        if (e[i].x != e[i-1].x)
        {
            ll val = t.getMin(1, cc, 1, L, R-1);
            if (val == 0)
            {
        ...
        cout << e[i].x << " " << e[i].y1 << " " << e[i].y2 - 1 << " " << e[i].type << "\n";
        if (e[i].type == 0)
            t.upd(1, cc, 1, e[i].y1, e[i].y2-1, 1);
        else t.upd(1, cc, 1, e[i].y1, e[i].y2-1, -1);
    }
    return 0;
}
int main()
